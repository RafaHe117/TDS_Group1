...
out_dir  <- "imputation"
out_file <- file.path(out_dir, "ukb_G1_imputed_iter5_tree75.rds")
...
# rebuild ukb_imp safely from current ukb
...
if (length(batch2_vars_main) > 0) {
  ...
  # update ukb_imp too, so oily run sees latest values
...
if (length(batch2_vars_oily) > 0) {
  # ensure it's plain factor
  if ("oily_fish_3cat" %in% names(ukb_imp)) {
    ukb_imp$oily_fish_3cat <- factor(ukb_imp$oily_fish_3cat, ordered = FALSE)
  }
  
  missing_oily <- setdiff(batch2_vars_oily, names(ukb_imp))
  if (length(missing_oily)) stop("oily_fish_3cat missing from ukb_imp unexpectedly.")
  
  # extra debug (goes to .out)

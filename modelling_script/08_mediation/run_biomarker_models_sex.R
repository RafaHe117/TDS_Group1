...
resolve_sex_mask <- function(sex_vector, sex_label, sex_value_override = NULL) {
  sex_label <- tolower(sex_label)
  
  if (!is.null(sex_value_override) && sex_value_override != "") {
    if (is.numeric(sex_vector)) {
      return(sex_vector == suppressWarnings(as.numeric(sex_value_override)))
    } else {
      return(as.character(sex_vector) == as.character(sex_value_override))
    }
  }
  
  if (is.factor(sex_vector) || is.character(sex_vector)) {
    sx <- tolower(as.character(sex_vector))
    if (sex_label == "male") return(sx %in% c("male", "m"))
    if (sex_label == "female") return(sx %in% c("female", "f"))
  }
  
  if (is.numeric(sex_vector)) {
    vals <- sort(unique(stats::na.omit(sex_vector)))
    if (identical(vals, c(0, 1))) {
      if (sex_label == "male") return(sex_vector == 1)
      if (sex_label == "female") return(sex_vector == 0)
    }
  }
  
  stop(
    "Could not infer sex coding automatically. Re-run with explicit override, e.g. male 1 or female 0."
  )
}
...
sex_value_override <- if (length(args) >= 2) args[2] else NULL

if (!sex_label %in% c("male", "female")) {
  stop("First argument must be 'male' or 'female'.")
}
...
selected_terms_path <- file.path(inputs_dir, paste0("selected_terms_", sex_label, ".csv"))
...
if (!"sex" %in% names(df)) {
  stop("Column 'sex' not found in data.")
}
...
write_csv(
  ...
  file.path(out_dir, paste0("detected_biomarkers_", sex_label, ".csv"))
)

if (length(biomarkers) == 0) {
  stop("No biomarker candidates available in data for ", sex_label)
}
...
message("Sex stratum: ", sex_label)
message("Rows in stratum: ", nrow(df))
...
if (length(results) == 0) {
  stop("No valid biomarker stability results were produced for ", sex_label, ".", call. = FALSE)
}
...
write_csv(
  final_res,
  file.path(out_dir, paste0("biomarker_stability_", sex_label, ".csv"))
)
...
write_csv(
  stable_links,
  file.path(out_dir, paste0("stable_links_", sex_label, ".csv"))
)

summary_df <- tibble(
  analysis = sex_label,
  n_rows_in_stratum = nrow(df),
...
write_csv(
  summary_df,
  file.path(out_dir, paste0("summary_", sex_label, ".csv"))
)

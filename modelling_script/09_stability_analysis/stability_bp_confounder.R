...
base_out_dir <- file.path(in_dir, "modelling_script", "09_stability_analysis", "subsample_lasso_bp_confounder")
...
# =========================
# Run sharp subsampling stability selection
# =========================
cat("Running sharp::VariableSelection with subsampling ...\n")
...
saveRDS(
  stab,
  file.path(base_out_dir, "stab_subsample.rds")
)
...
best_id  <- ArgmaxId(stab)

best_lambda <- best_par[1, 1]
best_pi     <- best_par[1, 2]

best_lambda_index <- best_id[1, 1]
best_pi_index     <- best_id[1, 2]
...
write.csv(
  sel_prop_df,
  file.path(base_out_dir, "selection_proportions_all_variables.csv"),
  row.names = FALSE
)
...
# =========================
# Refit and evaluate final model
# =========================
set.seed(999)
...
# =========================
# Save calibration heatmap
# =========================
png(
  filename = file.path(base_out_dir, "tuning_heatmap_sharp_subsample.png"),
  width = 12,
  height = 7.5,
  units = "in",
  res = 320
...
p_sel <- ggplot(plot_df, aes(x = term_plot, y = selection_proportion, color = selected)) +
  ...
  labs(
    title = "sharp subsampling: selection proportions",
    subtitle = paste0(
...
ggsave(
  file.path(base_out_dir, "selection_proportion_subsample.png"),
  p_sel,
  width = 14,
  height = 8.5,
  dpi = 320
...
if (!is.null(test_prob) && !anyNA(test_prob)) {
  ...
  ggsave(
    file.path(base_out_dir, "roc_test_subsample.png"),
    p_roc,
    width = 6.5,
    height = 6.5,
    dpi = 320
...
write.csv(
  sel_prop_best_pen,
  file.path(base_out_dir, "selection_proportions_penalized.csv"),
  row.names = FALSE
)
...
writeLines(
  summary_lines,
  file.path(base_out_dir, "summary.txt")
)